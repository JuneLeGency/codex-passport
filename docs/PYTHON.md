...
Python 3.10+；电脑 BLE 直连需要 `[ble]` 可选依赖。Python 中继 0.3.1 与 APK/固件 0.3.1 配套使用，本次 Python 更新仅涉及版本信息和文档；0.3.0 中继也能配合新版省电固件使用。升级时保留状态目录和绑定。
...
uvx --from 'codex-passport-sync[ble]==0.3.1' codex-passport serve --host "$COMPUTER_LAN_IP" --port 18765
```
...
```sh
uvx --refresh --index https://pypi.org/simple --from 'codex-passport-sync==0.3.1' codex-passport --help
uv tool install --index https://pypi.org/simple 'codex-passport-sync[ble]==0.3.1'
```
...
无法使用 PyPI 时，下载 [Python 发布文件](https://github.com/JuneLeGency/codex-passport/releases/tag/v0.3.1) 中的 wheel 和 `SHA256SUMS`，在文件所在目录运行：

```sh
uvx --from ./codex_passport_sync-0.3.1-py3-none-any.whl codex-passport --help
```

持久安装并启用电脑 BLE：

```sh
uv tool install 'codex-passport-sync[ble] @ ./codex_passport_sync-0.3.1-py3-none-any.whl'
uv tool update-shell
```
...
```sh
python3 -m venv .venv
.venv/bin/python -m pip install 'codex-passport-sync[ble]==0.3.1'
