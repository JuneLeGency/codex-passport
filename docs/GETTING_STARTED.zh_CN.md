...
打开 Codex Passport，先不用填配置。开启手机蓝牙，下一步先让电脑中继运行。
...
4. 点击“保存并连接”。允许“附近设备”和通知权限；Android 11 及更早版本还需定位权限与定位开关来扫描 BLE。
...
看到手机同步正常意味着已收到 Passport 回执，单纯“BLE 已连接”还只是连接建立。

## 7. 发一条真实通知验收

重新打开一个 Codex 会话，发送：“请只回复：Passport 通知测试完成”。等待这一轮结束。

预期：手机“通知”页出现这一会话的短摘要，Passport 未读铃铛数量增加；有新提醒时会亮屏。
